...
// This is the file defined at lexical/src/__tests__/utils.
...
export function initializeUnitTest(
  ...
  beforeEach(async () => {
    ...
    // FIX: The hard casting is necessary. `ref.current` will be provided in a
    // JSX portion.
...
export function expectHtmlToBeEqual(actual: string, expected: string): void {
  prettifyHtml(actual).then((actualHTML) => {
    prettifyHtml(expected).then((expectedHTML) => {
      expect(actualHTML).toBe(expectedHTML);
    });
  });
}

export async function prettifyHtml(s: string): Promise<string> {
  // FIX: Prettier has been removed. In general it is just a horrifically
  // disproportionate method to standardise HTML, but a replacement is
  // needed. The specific reason it has been removed is that it requires
  // a node flag and adding it seems to be overkill to do this one thing.
